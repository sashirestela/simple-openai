...
import io.github.sashirestela.openai.domain.response.events.ResponseStreamEvent;
...
public interface OpenAI {
    ...
    /**
     * Create model responses with the new responses API.
     *
    ...
    interface Responses {
        ...
        /**
         * Creates a model response for the given input without streaming (don't call it directly).
         *
         * @param responseRequest Includes the input and model to use.
         * @return The response object.
         */
        @POST
        CompletableFuture<io.github.sashirestela.openai.domain.response.Response> createPrimitive(
                @Body io.github.sashirestela.openai.domain.response.ResponseRequest responseRequest);

        /**
         * Creates a model response for the given input with streaming events (don't call it directly).
         *
         * @param responseRequest Includes the input and model to use.
        ...
        @POST
        @ResponseStreamEvent
        CompletableFuture<Stream<Event>> createStreamEventPrimitive(
                @Body io.github.sashirestela.openai.domain.response.ResponseRequest responseRequest);
        ...
        default CompletableFuture<io.github.sashirestela.openai.domain.response.Response> create(
                @Body io.github.sashirestela.openai.domain.response.ResponseRequest responseRequest) {
            var request = responseRequest.withStream(Boolean.FALSE);
            return createPrimitive(request);
        }
        ...
        default CompletableFuture<Stream<Event>> createStreamEvent(
                @Body io.github.sashirestela.openai.domain.response.ResponseRequest responseRequest) {
            var request = responseRequest.withStream(Boolean.TRUE);
            return createStreamEventPrimitive(request);
        }
         ...
         */
        @GET("/{responseId}")
        CompletableFuture<io.github.sashirestela.openai.domain.response.Response> retrieve(
