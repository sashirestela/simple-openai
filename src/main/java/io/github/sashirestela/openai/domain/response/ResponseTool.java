...
/**
 * Represents a tool definition for the Response API. Tools allow the model to perform specific
 * actions. The Response API has a different structure for function tools than the Chat Completions
 * API: - Function parameters are defined directly at the top level - Each tool has a type
 * (function, file_search, web_search_preview) - No nested "function" object is used (unlike Chat
 * Completions API) Example usage for function tools: ```java // Using builder ResponseTool tool =
 * ResponseTool.builder() .type(ResponseToolType.FUNCTION) .name("get_weather") .description("Get
 * the current weather for a location") .parameters(schema) .strict(true) .build(); // Using
 * convenience method ResponseTool tool = ResponseTool.functionTool( "get_weather", "Get the current
 * weather for a location", schema ); ``` Example for built-in tools: ```java // Web search tool
 * ResponseTool webSearch = ResponseTool.webSearchTool(); // File search tool ResponseTool
 * fileSearch = ResponseTool.fileSearchTool(); ```
 */
@Data
@NoArgsConstructor
@AllArgsConstructor
...
public class ResponseTool {
    ...
    public enum ResponseToolType {
        ...
        /**
         * Gives the model the ability to search the web. Currently available in the Response API.
         */
    ...
    /**
     * Creates a new function tool for the Response API. Default strict value is not set (null).
     *
     * @param name        The name of the function
     * @param description A description of what the function does
     * @param parameters  The JSON Schema object defining the function's parameters
    ...
    /**
     * Creates a web search tool for the Response API. This built-in tool gives the model the ability to
    ...
    /**
     * Creates a file search tool for the Response API with vector store IDs. This built-in tool allows
     * the model to search through files in the specified vector stores.
     *
    ...
    /**
     * Note: Code interpreter (computer_use_preview) is not yet available in the Response API. This
     * method will be implemented when the feature becomes available.
     */
}
